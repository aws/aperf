...
// TODO: GraphDataPage and the GraphData type are temporary, currently used only
//       by hotline. They will be replaced by new data formats.
...
function getAllGraphGroups(dataType: DataType): SegmentedControlProps.Option[] {
  ...
  for (const runName of RUNS) {
    const reportData = PROCESSED_DATA[dataType]?.runs?.[runName] as GraphData | undefined;
    if (!reportData?.graph_groups) continue;
...
function getGraphNames(dataType: DataType, graphGroupName: string): string[] {
  ...
  for (const runName of RUNS) {
    ...
    if (graphGroup == undefined || !graphGroup.graphs) continue;
