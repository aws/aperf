  ...
  interrupts: {
    ...
    fieldDescriptions: {
      "CAL (Function call interrupts)": {
        ...
        description: "Inter-processor interrupts used to execute functions on remote CPUs. These are essential for SMP coordination and workload distribution across cores.",
        desired: "depends", 
      },
      "DFR (Deferred Error APIC interrupts)": {
        readableName: "Deferred Error APIC Interrupts",
        description: "APIC interrupts for handling deferred machine check errors that don't require immediate attention. These allow the system to process non-critical hardware errors without blocking normal operation.",
        desired: "lower", 
      },
      "ERR": {
        readableName: "Error Count",
        description: "Counter for various interrupt controller errors and spurious interrupts. This tracks interrupt delivery failures and hardware anomalies in the interrupt subsystem.",
        desired: "lower", 
      },
      "Err": {
        readableName: "Error Count",
        description: "Alternative error counter used on some architectures for interrupt-related errors. Similar to ERR but may track different types of interrupt subsystem failures.",
        desired: "lower", 
      },
      "HYP (Hypervisor callback interrupts)": {
        readableName: "Hypervisor Callback Interrupts",
        description: "Interrupts generated by hypervisor for communication with guest operating systems. These facilitate virtualization operations and host-guest coordination in virtualized environments.",
        desired: "depends", 
      },
      "IPI0 (Rescheduling interrupts)": {
        readableName: "Rescheduling Interrupts",
        description: "Inter-processor interrupts that trigger CPU rescheduling on remote cores. These ensure proper load balancing and task migration across multiple processors.",
        desired: "depends", 
      },
      "IPI1 (Function call interrupts)": {
        readableName: "Function Call Interrupts",
        description: "Inter-processor interrupts for executing specific functions on target CPUs. These enable cross-CPU synchronization and distributed processing operations.",
        desired: "depends", 
      },
      "IPI2 (CPU stop interrupts)": {
        readableName: "CPU Stop Interrupts",
        description: "Inter-processor interrupts used to halt specific CPUs during system shutdown or maintenance. These provide controlled CPU shutdown for system management operations.",
        desired: "lower", 
      },
      "IPI3 (CPU stop (for crash dump) interrupts)": {
        readableName: "CPU Stop for Crash Dump Interrupts",
        description: "Inter-processor interrupts that stop CPUs to enable crash dump collection. These ensure system stability during kernel panic and crash analysis procedures.",
        desired: "lower", 
      },
      "IPI3 (CPU stop NMIs)": {
        readableName: "CPU Stop NMIs",
        description: "Non-maskable inter-processor interrupts for emergency CPU halt operations. These provide the highest priority mechanism to stop CPUs during critical system failures.",
        desired: "lower", 
      },
      "IPI4 (Timer broadcast interrupts)": {
        readableName: "Timer Broadcast Interrupts",
        description: "Inter-processor interrupts for broadcasting timer events across CPUs. These maintain time synchronization and coordinate timer-based operations in multi-processor systems.",
        desired: "depends", 
      },
      "IPI5 (IRQ work interrupts)": {
        readableName: "IRQ Work Interrupts",
        description: "Inter-processor interrupts for scheduling work items on remote CPUs. These enable deferred work processing and cross-CPU task delegation.",
        desired: "depends", 
      },
      "IPI6 (CPU backtrace interrupts)": {
        readableName: "CPU Backtrace Interrupts",
        description: "Inter-processor interrupts that trigger stack trace collection from remote CPUs. These are used for debugging and system analysis during development or troubleshooting.",
        desired: "lower", 
      },
      "IPI6 (CPU wake-up interrupts)": {
        readableName: "CPU Wake-up Interrupts",
        description: "Inter-processor interrupts used to wake up idle or sleeping CPUs. These enable dynamic CPU power management and on-demand processor activation.",
        desired: "depends", 
      },
      "IPI7 (KGDB roundup interrupts)": {
        readableName: "KGDB Roundup Interrupts",
        description: "Inter-processor interrupts used by kernel debugger to synchronize all CPUs. These facilitate kernel debugging by ensuring all processors are in a known state.",
        desired: "lower", 
      },
      "IWI (IRQ work interrupts)": {
        readableName: "IRQ Work Interrupts",
        description: "Interrupts for processing deferred IRQ work items on the current CPU. These handle interrupt-related tasks that cannot be completed in interrupt context.",
        desired: "depends", 
      },
      "LOC (Local timer interrupts)": {
        readableName: "Local Timer Interrupts",
        description: "Timer interrupts generated by each CPU's local APIC timer. These drive the scheduler tick and maintain per-CPU timing for process scheduling and system timekeeping.",
        desired: "depends", 
      },
      "MCE (Machine check exceptions)": {
        readableName: "Machine Check Exceptions",
        description: "Hardware-generated interrupts for reporting serious CPU and system errors. These indicate potential hardware failures that require immediate attention or system shutdown.",
        desired: "lower", 
      },
      "MCP (Machine check polls)": {
        readableName: "Machine Check Polls",
        description: "Periodic interrupts for polling machine check status registers. These proactively monitor hardware health and detect errors before they become critical.",
        desired: "lower", 
      },
      "MIS": {
        readableName: "Miscellaneous Interrupts",
        description: "Counter for various uncategorized interrupt events. This tracks interrupt activity that doesn't fit into other specific categories.",
        desired: "lower", 
      },
      "NMI (Non-maskable interrupts)": {
        readableName: "Non-Maskable Interrupts",
        description: "High-priority interrupts that cannot be disabled by software. These handle critical system events like hardware failures and debugging requests.",
        desired: "lower", 
      },
      "NPI (Nested posted-interrupt event)": {
        readableName: "Nested Posted-Interrupt Event",
        description: "Virtualization-specific interrupts for handling nested interrupt posting. These optimize interrupt delivery in nested virtualization environments.",
        desired: "depends", 
      },
      "PIN (Posted-interrupt notification event)": {
        readableName: "Posted-Interrupt Notification Event",
        description: "Virtualization interrupts for notifying about posted interrupt availability. These improve interrupt handling efficiency in virtual machine environments.",
        desired: "depends", 
      },
      "PIW (Posted-interrupt wakeup event)": {
        readableName: "Posted-Interrupt Wakeup Event",
        description: "Virtualization interrupts for waking up CPUs to handle posted interrupts. These optimize interrupt processing in virtualized systems with sleeping CPUs.",
        desired: "depends", 
      },
      "PMI (Performance monitoring interrupts)": {
        readableName: "Performance Monitoring Interrupts",
        description: "Interrupts generated by CPU performance monitoring units. These enable profiling and performance analysis by sampling CPU events and counters.",
        desired: "depends", 
      },
      "RES (Rescheduling interrupts)": {
        readableName: "Rescheduling Interrupts",
        description: "Interrupts that trigger process rescheduling on the current CPU. These maintain fair scheduling and respond to priority changes in the system.",
        desired: "depends", 
      },
      "RTR (APIC ICR read retries)": {
        readableName: "APIC ICR Read Retries",
        description: "Counter for APIC Interrupt Command Register read retry operations. This tracks communication reliability between APIC controllers in multi-processor systems.",
        desired: "lower", 
      },
      "SPU (Spurious interrupts)": {
        readableName: "Spurious Interrupts",
        description: "False interrupts generated due to electrical noise or timing issues. These represent interrupt controller glitches that don't correspond to actual interrupt sources.",
        desired: "lower", 
      },
      "THR (Threshold APIC interrupts)": {
        readableName: "Threshold APIC Interrupts",
        description: "APIC interrupts triggered when error counters exceed predefined thresholds. These provide early warning for accumulating hardware errors before they become critical.",
        desired: "lower", 
      },
      "TLB (TLB shootdowns)": {
        readableName: "TLB Shootdowns",
        description: "Inter-processor interrupts for invalidating Translation Lookaside Buffer entries. These maintain memory coherency by ensuring stale page translations are removed across all CPUs.",
        desired: "depends", 
      },
      "TRM (Thermal event interrupts)": {
        readableName: "Thermal Event Interrupts",
        description: "Interrupts generated when CPU temperature exceeds safe operating limits. These trigger thermal throttling and cooling measures to prevent hardware damage.",
        desired: "lower", 
      },
    },
  },
